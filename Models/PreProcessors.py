...
class WindowProcessor(PreProcessors):
    ...
    def transform(self, X, y, n_in=1, n_out=1, dropnan=True, y_name=['val_vaz_natr', 'd_vazao', 'd2_vazao']):
        X = pd.DataFrame(X)
        y = pd.DataFrame(y)
        ...
        for y in y_name:
            col_names = ['{var:}_(t{lag:+d})'.format(var=y, lag=i) for i in range(0, n_out)]
            X_lag.drop(columns=col_names, inplace=True)

        # Remove t-n_in ... t-1 da variavel y de y_lag
        nomes_y = ['vazao_(t+{:})'.format(i) for i in range(0, n_out)]
        y_lag = y_lag.loc[:, nomes_y]
